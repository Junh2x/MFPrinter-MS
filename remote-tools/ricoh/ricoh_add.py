...
def add_destination(s, wim_token, reg_no, name, display_name, folder_path, folder_user, folder_pass):
    ...
    r = s.post(f"{BASE}/web/entry/ko/address/adrsGetUserWizard.cgi",
               data={"mode": "ADDUSER", "outputSpecifyModeIn": "DEFAULT", "wimToken": wim_token},
    ...
    r = s.post(url, headers=h, timeout=10,
               data={"mode": "ADDUSER", "step": "FAX", "wimToken": wim_token, "faxDestIn": ""})
    log(f"[Step2] FAX → {r.status_code} ({r.text.strip()[:50]})")

    # Step 3: FOLDER
    pw = b64(folder_pass) if folder_pass else ""
    r = s.post(url, headers=h, timeout=10, data={
        "mode": "ADDUSER", "step": "FOLDER", "wimToken": wim_token,
    ...
    r = s.post(url, headers=h, timeout=10, data=[
        ...
        ("stepListIn", "FOLDER"), ("mode", "ADDUSER"), ("step", "CONFIRM"),
    ])
