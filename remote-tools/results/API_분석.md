...
- CSRF 토큰 외 다른 검증 있는 지 확인 필요

캐논 => 
1. CSRF 토큰, IR, 세션 3차 검증 확인, HTTP REQ로 등록 테스트 성공
2. AID 값에 따라 목록 파싱 실패 케이스 발생
3. HTTP 수신지 등록 성공했으므로, 요구사항별 검증 필요
4. 등록 프로세스 간소화 가능성 검토 필요
